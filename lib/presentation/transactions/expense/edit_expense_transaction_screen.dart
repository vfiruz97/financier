...
import 'dialog/enter_balance_dialog.dart';
...
class EditExpenseTransactionScreen extends StatelessWidget {
  const EditExpenseTransactionScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final transactionId = context.read<ExpenseTransactionUpdateBloc>().state.transaction.id.getOrCrash();
                                ...
                                onTap: () async {
                                  ...
                                  if (response != null && response) {
                                    BlocProvider.of<ExpenseTransactionUpdateBloc>(context)
                                        .add(const ExpenseTransactionUpdateEvent.delete());
                            ...
                            child: Column(
                              ...
                              children: [
                                Padding(
                                  ...
                                  child: Text('Expense', style: Theme.of(context).textTheme.titleSmall),
                                ),
                                ListTransactionItem(
                                  fieldName: 'Account',
                                  value:
                                      '${account?.name.getOrElse('')}', //${account?.balance.getOrElse(0)} ${account?.currencyId.code}
                                  ...
                                  onTap: () async {
                                    await showDialog(
                                      ...
                                      builder: (context) => ChooseCategoryDialog(
                                        onSelect: (categoryId) {
                                          BlocProvider.of<ExpenseTransactionUpdateBloc>(context)
                                            ..add(ExpenseTransactionUpdateEvent.changeCategoryId(categoryId))
                                            ..add(const ExpenseTransactionUpdateEvent.update());
                                        },
                                      ),
                                    );
