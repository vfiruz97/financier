...
class EnterBalanceForm extends StatefulWidget {
  const EnterBalanceForm({super.key, required this.onTap, this.onBack});
...
class _EnterBalanceFormState extends State<EnterBalanceForm> {
  ...
  @override
  void initState() {
    currencyCode = context.read<TransferTransactionCreateBloc>().state.accountFrom.currencyId.code;
    final balance = context.read<TransferTransactionCreateBloc>().state.expenseTransaction.balance.getOrElse(0);
    amount = balance > 0
        ? '$balance'.endsWith('.0')
      ...
      child: Stack(
        children: [
          Positioned(
              ...
              top: 0,
              right: 0,
              child: BackAppBar(title: 'Amount to get', onBack: widget.onBack != null ? () => widget.onBack!() : null)),
          Positioned(
            left: 0,
            top: 60,
            right: 0,
                            ...
                            child: InkWell(
                              onTap: () {
                                BlocProvider.of<TransferTransactionCreateBloc>(context)
                                    .add(TransferTransactionCreateEvent.fixAmount(double.parse(amount)));
                                widget.onTap();
                              },
                              child: const Padding(
