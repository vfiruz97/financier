...
class PutAmountInForm extends StatefulWidget {
  const PutAmountInForm({super.key, required this.onTap, this.onBack});

  final Function() onTap;
  final Function()? onBack;

  @override
  State<PutAmountInForm> createState() => _EnterBalanceFormState();
}

class _EnterBalanceFormState extends State<PutAmountInForm> {
  late String currencyCode;
  String amount = '0';

  @override
  void initState() {
    currencyCode = context.read<TransferTransactionCreateBloc>().state.accountTo.currencyId.code;
    final balance = context.read<TransferTransactionCreateBloc>().state.incomeTransaction.balance.getOrElse(0);
    amount = balance > 0
        ? '$balance'.endsWith('.0')
      ...
      child: Stack(
        children: [
          Positioned(
              ...
              top: 0,
              right: 0,
              child: BackAppBar(title: 'Amount to put', onBack: widget.onBack != null ? () => widget.onBack!() : null)),
          Positioned(
            left: 0,
            top: 60,
            right: 0,
                            ...
                            child: InkWell(
                              onTap: () {
                                BlocProvider.of<TransferTransactionCreateBloc>(context)
                                  ..add(TransferTransactionCreateEvent.exchangeAmount(double.parse(amount)))
